...
const pvpLvl = [
  0, 0, 2000, 4000, 6000, 8000, 11000, 14000, 17000, 20000, 23000, 27000, 31000,
  35000, 39000, 43000, 48500, 54000, 59500, 65000, 70500, 78000, 85500, 93000,
  100500, 108000, 118000, 128000, 138000, 148000, 158000, 178000, 198000,
...
function loadInputData() {
  const inputData =
    JSON.parse(localStorage.getItem("MalmstoneXPCalculator")) || {};
...
function calculateXP() {
  ...
  if (isNaN(current_level) || isNaN(goal_level) || isNaN(current_progress)) {
    ...
    !outputDiv?.classList.contains("hidden")
  ...
  const cctotal_Win =
    Math.floor(exp / CrystalineWin_Exp) + (exp % CrystalineWin_Exp > 0 ? 1 : 0);
  const cctotal_Lose =
    Math.floor(exp / CrystalineLose_Exp) +
    (exp % CrystalineLose_Exp > 0 ? 1 : 0);
  const fltotal_Win =
    Math.floor(exp / frontlineWin_Exp) + (exp % frontlineWin_Exp > 0 ? 1 : 0);
  const fltotal_Lose =
    Math.floor(exp / frontlineLose_Exp) + (exp % frontlineLose_Exp > 0 ? 1 : 0);
  const fltotal_Lose2 =
    Math.floor(exp / frontlineLose2_Exp) +
    (exp % frontlineLose2_Exp > 0 ? 1 : 0);
  const fltotal_DailyWin =
    Math.floor(exp / frontlineDailyWin_Exp) +
    (exp % frontlineDailyWin_Exp > 0 ? 1 : 0);
  const fltotal_DailyLose =
    Math.floor(exp / frontlineDailyLose_Exp) +
    (exp % frontlineDailyLose_Exp > 0 ? 1 : 0);
  const fltotal_DailyLose2 =
    Math.floor(exp / frontlineDailyLose2_Exp) +
    (exp % frontlineDailyLose2_Exp > 0 ? 1 : 0);
  const rwtotal_Win =
    Math.floor(exp / rivalwingsWin_Exp) + (exp % rivalwingsWin_Exp > 0 ? 1 : 0);
  const rwtotal_Lose =
    Math.floor(exp / rivalwingsLose_Exp) +
    (exp % rivalwingsLose_Exp > 0 ? 1 : 0);
...
function copyImportExportString() {
  const input = document.getElementById("importExportString");
...
function exportData() {
  const raw = localStorage.getItem("MalmstoneXPCalculator") || "{}";
  const encoded = btoa(raw);
  const textarea = document.getElementById("importExportString");
...
function importData() {
  const statusEl = document.getElementById("importStatus");
  const encoded = document.getElementById("importExportString").value.trim();
  if (!encoded) {
    statusEl.style.color = "var(--color-error, red)";
    statusEl.textContent = "⚠ Please paste an export string before importing.";
    return;
  }
  try {
    const raw = atob(encoded);
    JSON.parse(raw); // validate it is well-formed JSON before saving
    localStorage.setItem("MalmstoneXPCalculator", raw);
    loadInputData();
    calculateXP();
    statusEl.style.color = "var(--color-success, green)";
    ...
    statusEl.textContent =
      "⚠ Invalid export string. Please check and try again.";
  }
}
